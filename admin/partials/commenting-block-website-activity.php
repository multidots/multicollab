...
if ( 'detail-view' === $activity_view ) { ?>
    <div class="board-detail-head">
        <a href="javascript::void(0)" id="activity-go-back">Go Back</a>
...
foreach ( $this->cf_activities as $meta_single ) { //phpcs:ignore  
	...
	if ( strpos( $meta_key, 'el' ) !== false ) {
		...
		$meta             = [];
		$meta['comments'] = $suggestions_meta[ $suggestion_key ];

		// Handle resolved suggestions.
		$top_suggestion = $suggestions_meta[ $suggestion_key ][0];
		if ( isset( $top_suggestion['status'] ) && isset( $top_suggestion['status']['timestamp'] ) ) {
			...
			$meta['resolved_text']      = 'accept' === $top_suggestion['status']['action'] ? 'Suggestion Accepted' : 'Suggestion Rejected';
		}
	}
	...
	if ( $main_timestamp > $beginning_of_week ) {
		if ( gmdate( 'Y-m-d', $main_timestamp ) === gmdate( 'Y-m-d', $now ) ) {
			$date_displaying = 'Today';
		} else if ( gmdate( 'Y-m-d', $main_timestamp ) === gmdate( 'Y-m-d', $yesterday ) ) {
			$date_displaying = 'Yesterday';
		}
	}
	...
	if ( ! in_array( $date_displaying, $date_displayed, true ) && ! $date_continue_in_loadmore ) {
		...
		echo "<h3 class='board-items-day'>".esc_html($date_displaying)."</h3>";
		$new_date_displayed = true;
	...
	if ( is_array( $all_comments ) ) {
		...
		if ( isset($meta['resolved_by']) ) {
			...
			$all_comments['resolved']['resolved_text']      = isset( $meta['resolved_text'] ) ? $meta['resolved_text'] : 'Marked as resolved.';
		}
		...
		?>
        <div class="board-cnt-main <?php esc_attr_e( $board_position );
		echo $date_continue_in_loadmore ? ' date_continued' : ''; ?> <?php echo $post_continue_in_loadmore ? 'post_continued' : ''; ?>">
                ...
                <div class="board-cnt-wrap">
                    ...
                    <div class="user-data-row">
						...
						foreach ( $all_comments as $timestamp => $comment ) {  //phpcs:ignore
							...
							$date_time = gmdate( $time_format . ' ' . $date_format, $timestamp );							 
                            ?>
                                ...
                                <div class="user-data">
									...
									<?php if ( isset($comment['resolved_timestamp']) ) { ?>
                                        <div class="user-comment <?php echo 'comments' === $board_type ? 'mark-resolved-icon' : '' ; ?>">
                                            <strong><?php esc_html_e( $comment['resolved_text'] ); ?></strong>
                                        </div>
									<?php } else { ?>
                                        <div class="user-data-wrapper">
											<?php if ( 1 === $comments_counts && 'comments' === $board_type ) { ?>
                                                <div class="user-commented-on">
                                                    <blockquote class="user-commented-icon<?php echo ($meta['blockType']) ? esc_attr('-'.$meta['blockType']) : '' ?>">
											...
											<?php }elseif ( 'suggestions' === $board_type && isset($comment['action']) && 'reply' !== $comment['action'] ) { ?>
                                                <div class="user-commented-on <?php echo  (isset($comment['action']) && 'delete' === strtolower($comment['action'])) ? 'delete' : 'add' ; ?>">
                                                    <blockquote class="user-commented-icon">
														...
														echo '<strong>'.esc_html($comment['action']).'</strong> : ' ; ?>
														<a class="user-commented-on show-all <?php echo ( '' !== $comment['text'] && strlen(wp_strip_all_tags($comment['text']))  > 45 ) ? 'js-hide' : '' ; ?>" href="javascript:void(0)" >
												    		<?php echo esc_html(wp_strip_all_tags($comment['text']));  ?>
														</a>
											...
											<div class="user-assigned-to"><span class="icon"></span><span class="assign-avatar-data">Assigned to<a href="mailto:<?php echo esc_attr($assigned_to['user_email']); ?>" title="<?php echo esc_attr($assigned_to['display_name']); ?>"> <?php echo esc_html($assigned_to['display_name']); ?></a></span></div>
											<?php } ?>
											<?php if ( isset( $comment['editedTime'] ) && 'deleted' !== $comment['status'] ) {
												...
												$edited_time = gmdate( $side_time_format, $timestamp ); ?>
                                                <time class="user-commented-date user-inner-box-time">(edited <?php esc_html_e( $edited_time ); ?>)</time>
											<?php } ?>
                                        </div>
									<?php } ?>
                                </div>
							...
							if( !$showallcommentbtnflag && $index > 6  && count ( $all_comments ) > 7 ){
								...
								?>
								<div class="show-all-comments">Show all <?php echo esc_html( $count ); ?> replies</div>
								<?php
							}
