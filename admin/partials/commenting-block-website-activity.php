								...
								<div class="<?php echo esc_attr( $box_class ); ?>" style="<?php echo ( $index > 6 && ! isset( $comment['resolved_by'] ) ) ? 'display:none' : ''; ?>">
									<div class="user-data">
										...
										if ( cf_fs()->is__premium_only() && cf_fs()->is_plan( 'vip', true ) ) { 
											if ( isset( $comment['attachmentText'] ) && $comment['attachmentText'] !== '' ) {
												?>
												<div class="user-comment-attchment">
												<?php echo wp_kses( $comment['attachmentText'], wp_kses_allowed_html( 'post' ) ); ?>
												</div>	
												<?php
											}
										}
...
?>
<?php
if ( 'list-view' === $activity_view && !empty( $main_timestamp ) ) {
